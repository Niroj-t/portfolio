    ...
    <div className="hidden h-9 shrink-0 overflow-x-auto overscroll-x-contain border-b border-tn-border bg-tn-bg-dark [-webkit-overflow-scrolling:touch] sm:flex">
      {tabs.map((tab) => (
        <div
          ...
          className={`group flex max-w-[45vw] shrink-0 cursor-pointer items-center gap-1.5 border-r border-tn-border px-2 text-[12px] transition-colors sm:max-w-none sm:gap-2 sm:px-3 sm:text-[13px] ${
            activeTab === tab
              ? "border-t-2 border-t-tn-blue bg-tn-bg text-tn-fg shadow-[inset_0_1px_0_rgba(122,162,247,0.6),0_2px_12px_-4px_rgba(122,162,247,0.3)]"
              : "bg-tn-bg-dark text-tn-muted hover:bg-tn-bg-hover hover:text-tn-cyan"
          }`}
